...
function Pipeline:apply(code, filename)
	...
	if(self.Seed > 0) then
		...
		local success, seed = pcall(function()
			...
			if isWindows then
				-- Windows: use multiple %RANDOM% calls (each gives 0-32767)
				local handle = io.popen("cmd /c echo %RANDOM%%RANDOM%%RANDOM%%RANDOM%%RANDOM%%RANDOM%")
				seedStr = handle:read("*a"):gsub("%s+", "")
				handle:close()
			else
				seedStr = io.popen("openssl rand -hex 12"):read("*a"):gsub("\n", "")..""
			end
			local seedNum = 0;
